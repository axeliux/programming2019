class Solution {
    public int networkDelayTime(int[][] times, int N, int K) {
        ...
        // if there is not edges from the source, simplly return -1
        
        if (g.getAdj(K).isEmpty()) return -1; 
    ...
    public class SP {
        ...
        public int longestDistance() {
            ...
            int max = 0;
            int maxIndex = 0;
            boolean sombodyUnreachable = false;
            for(int i = 1; i < distTo.length; i++) {
                if (distanceTo(i) < 0) {
                    sombodyUnreachable = true;
                }
                if (max < distanceTo(i)) {
                    max = distanceTo(i);
                    maxIndex = i;
            ...
            printPath(maxIndex);
            return  (!sombodyUnreachable?  max: -1);
        }
    }
